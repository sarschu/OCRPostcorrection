...
class Normalizer(object):
    ...
    def __init__(self,filtering,lm_model,language, SMT_token, SMT_token2, SMT_unigram, SMT_unigram2, SMT_bigram, SMT_decision, MOSES_PATH, MOSES_SERVER, CRF_PATH, LM_PATH, SETTING, modules,dev,NNfile,KENLM,eval_dir="no_eval",**kwargs):
        ...
        **parameters**, **types**::
        ...
        if self.language =="en":
            self.hobj = hunspell.HunSpell('../static/hunspell/en_GB.dic', '../static/hunspell/en_GB.aff')
        elif self.language =="nl":
            self.hobj = hunspell.HunSpell('/usr/share/myspell/nl_NL.dic', '/usr/share/myspell/nl_NL.aff')
        elif self.language =="fr":
            self.hobj = hunspell.HunSpell('../static/hunspell/fr.dic', '../static/hunspell/fr.aff')
        elif self.language =="de":
            self.hobj = hunspell.HunSpell('../static/hunspell/de.dic', '../static/hunspell/de.aff')
    ...
    def start_server_mode(self):
        ...
        if u'SMT_Token' in self.modules1 or u'SMT_Cascaded' in self.modules1:
            ...
            p1 = subprocess.Popen(self.MOSES_SERVER+" -f "+self.SMT_tokmodel +" --server-port "+ port_tok +" --server-log "+util.LOG_DIR+"/server/moses_server_token.log",stdout=log,stderr=subprocess.PIPE,shell=True,bufsize=-1,env={"LD_LIBRARY_PATH":"/mount/projekte/sfb-732/inf/users/sarah/tools/xmlrpc-c-1.39.12/lib"},universal_newlines=True, preexec_fn=os.setsid)
            ...
            if elapsed_time >= 250:
                ...
                sys.exit()
            self.s_token = x.ServerProxy("http://localhost:"+port_tok+"/RPC2",allow_none=True,verbose=True)

        if u'SMT_Token2' in self.modules1:
            normalizer_log.info("start token2 server mode")
            log = open(logfile,"a")
            port_tok2= str(random.randint(40000, 50000))
            print self.MOSES_SERVER+" -f "+self.SMT_tokmodel2 +" --server-port "+ port_tok2 +" --server-log "+util.LOG_DIR+"/server/moses_server_token2.log"
            p2 = subprocess.Popen(self.MOSES_SERVER+" -f "+self.SMT_tokmodel2 +" --server-port "+ port_tok2 +" --server-log "+util.LOG_DIR+"/server/moses_server_token2.log",stdout=log,stderr=subprocess.PIPE,bufsize=-1,shell=True,env={"LD_LIBRARY_PATH":"/mount/projekte/sfb-732/inf/users/sarah/tools/xmlrpc-c-1.39.12/lib"},universal_newlines=True)
            log.close()
            self.pids.append(p2)
            start_time = time.time()
            ...
            if elapsed_time >= 250:
                ...
                sys.exit()
            self.s_token2 = x.ServerProxy("http://localhost:"+port_tok2+"/RPC2",allow_none=True,verbose=True)
        ...
        if u'SMT_Unigram' in self.modules1 or u'SMT_Cascaded' in self.modules1:
            ...
            p3 = subprocess.Popen(self.MOSES_SERVER+" -f "+ self.SMT_unimodel+' --server-port '+ port_uni +" --server-log "+util.LOG_DIR+"/server/moses_server_unigram.log",stdout= log,stderr=subprocess.PIPE,bufsize=-1,shell=True,env={"LD_LIBRARY_PATH":"/mount/projekte/sfb-732/inf/users/sarah/tools/xmlrpc-c-1.39.12/lib"},universal_newlines=True, preexec_fn=os.setsid)
            log.close()
            self.pids.append(p3)
            elapsed_time=0
            start_time = time.time()
            ...
            if elapsed_time >= 250:
                ...
                sys.exit()
            self.s_unigram = x.ServerProxy("http://localhost:"+port_uni+"/RPC2",allow_none=True,verbose=True)                
        ...
        if u'SMT_Unigram2' in self.modules1:
            ...
            p4 = subprocess.Popen(self.MOSES_SERVER+" -f "+ self.SMT_unimodel2+" --server-port "+ port_uni2 +" --server-log "+util.LOG_DIR+"/server/moses_server_unigram2.log",stdout= log,stderr=subprocess.PIPE,bufsize=-1,shell=True,env={"LD_LIBRARY_PATH":"/mount/projekte/sfb-732/inf/users/sarah/tools/xmlrpc-c-1.39.12/lib"},universal_newlines=True)
            log.close()
            ...
            if elapsed_time >= 250:
                ...
                sys.exit()
            self.s_unigram2 = x.ServerProxy("http://localhost:"+port_uni2+"/RPC2",allow_none=True,verbose=True)     
                       
        if u'SMT_Bigram' in self.modules1:
            normalizer_log.info("start bigram server mode")
            log = open(logfile,"a")
            port_bi=str(random.randint(40000, 50000))
            p5 = subprocess.Popen(self.MOSES_SERVER+" -f "+self.SMT_bimodel +" --server-port "+ port_bi +" --server-log "+util.LOG_DIR+"/server/moses_server_bigram.log",stdout= log,stderr=log,shell=True,bufsize=-1,env={"LD_LIBRARY_PATH":"/mount/projekte/sfb-732/inf/users/sarah/tools/xmlrpc-c-1.39.12/lib"},universal_newlines=True)
            log.close()
            #p5.communicate()
            self.pids.append(p5)
            time.sleep(15)
            self.s_bigram = x.ServerProxy("http://localhost:"+port_bi+"/RPC2",allow_none=True,verbose=True)
    ...
    def _kill_server_mode(self,proc):
        ...
        **parameters**, **types**::
        ...
        for p in proc:
            #id1 =os.getpgid(p.pid)
            #os.killpg(id1,signal.SIGTERM)
            sid=p.pid
            os.kill(sid,9)
    ...
    def _normalize_sentence(self, sentence,ori_sentence):
        ...
        **parameters**, **types**,**return**,**return types**::
        ...
        #(u'females', {'module': [<modules.wordsplit.Word_Split object at 0x2c03e10>, <modules.wordsplit.Word_Split object at 0x2c03e10>, <modules.compound.Compound object at 0x2c03e90>, <modules.compound.Compound object at 0x2c03e90>, <modules.original.Original object at 0x2c03f50>, <modules.original.Original object at 0x2c03f50>, <modules.abbreviation.Abbreviation object at 0x2c12190>, <modules.abbreviation.Abbreviation object at 0x2c12190>, <modules.smt.SMT_Token object at 0x2c12b10>, <modules.smt.SMT_Token object at 0x2c12b10>, <modules.smt.SMT_Unigram object at 0x2c12cd0>, <modules.smt.SMT_Unigram object at 0x2c12cd0>, <modules.smt.SMT_Bigram object at 0x2c97350>, <modules.smt.SMT_Bigram object at 0x2c97350>, <modules.smt.SMT_Cascaded object at 0x3ec0210>, <modules.smt.SMT_Cascaded object at 0x3ec0210>, <modules.transliterate_new_unbal.Transliterate_new_unbal object at 0x3ec0250>, <modules.transliterate_new_unbal.Transliterate_new_unbal object at 0x3ec0250>, <modules.spellcheck.Hunspell object at 0x3ec0290>, <modules.spellcheck.Hunspell object at 0x3ec0290>], 'ori': [u'females']})
            ...
            #compile the phrase_dict which will later result in the phrase table
            #if you have alternatives that look the same but where suggested for 
            #different ori words, you have to append a random number to keep them apart
        ...
        print "alignment that goes to webservice"
        print self.result_untokenized
        normalizer_log.debug("SHARED TASK START")
        normalizer_log.debug(self.result_untokenized)
        normalizer_log.debug("SHARED TASK END")
    ...
    def _call_moses(self, s, phrase_table):
        ...
        **parameters**, **types**,**return**,**return types**::
        ...
        #add lines to the file in this variable self.SMT_decision
        #make a new file copy contents of self.SMT_decision and add:
        template_file = open(self.SMT_decision).read()
        new_line = 'PhraseDictionaryMemory name=TranslationModel0 num-features='+str(len(self.modules)+self.feature_increase)+' path='+phrase_table+' input-factor=0 output-factor=0'
        if "LM" in self.modules1:
            new_line += '\nKENLM name=LM1 factor=0 path=../static/Moses/lm/'+self.seclm+'order=5'
        changed_template= template_file.replace('[feature]','[feature]\n' + new_line)
        new_fname = self.SMT_decision+'_temp'
        new_file = open(new_fname, 'w')
        new_file.write(changed_template)
        new_file.close()
        
        logfile = util.logfile
        log = open(logfile,"a")
        echo = subprocess.Popen(['echo',s.encode("utf8")],stdout=subprocess.PIPE)
        proc = subprocess.Popen([self.MOSES_PATH,'-v','2','--distortion-limit','0', '-f', new_fname],stdin = echo.stdout, stdout= subprocess.PIPE, stderr= subprocess.PIPE,shell=False)
