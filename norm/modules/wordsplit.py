        ...
        for w in range(0,len(split_sent)):
            if len(split_sent[w]) >1 and w in corr_list:
                ...
                if self.normalizer.language =="nl":
                    ...
                    decomp = subprocess.Popen(["perl",util.STATIC_DIR+"/decompounder/compound-splitter.perl","-min-size", "2", "-model", util.STATIC_DIR+"/decompounder/decompound-en-gutenberg"],stdout=subprocess.PIPE,stdin=subprocess.PIPE,shell=False)
                elif self.normalizer.language =="de":
                    decomp = subprocess.Popen(["perl",util.STATIC_DIR+"/decompounder/compound-splitter.perl","-min-size", "2", "-model", util.STATIC_DIR+"/decompounder/gutenberg.model"],stdout=subprocess.PIPE,stdin=subprocess.PIPE,shell=False)
                elif self.normalizer.language =="fr":
                    decomp = subprocess.Popen(["perl",util.STATIC_DIR+"/decompounder/compound-splitter.perl","-min-size", "2", "-model", util.STATIC_DIR+"/decompounder/decompound-fr-gutenberg"],stdout=subprocess.PIPE,stdin=subprocess.PIPE,shell=False)
